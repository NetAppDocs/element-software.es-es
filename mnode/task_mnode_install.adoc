...
Antes de seguir este procedimiento, debe tener conocimientos de link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["Volúmenes persistentes"] y si desea usarlos o no. Los volúmenes persistentes son opcionales, pero se recomienda para la recuperación de datos de configuración del nodo de gestión en caso de pérdida de una máquina virtual (VM).
...
. Descargue el OVA o ISO para su instalación desde link:https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["Software Element"^] En el sitio de soporte de NetApp.
+
