...
[NOTE]
====
* A partir de noviembre de 2023, no podrá iniciar una actualización de componentes utilizando NetApp Hybrid Cloud Control o la API REST porque los certificados de clave de firma (privada y pública) caducaron el 5 de noviembre de 2023. Puedes solucionar este problema mediantelink:task_hcc_upgrade_management_node.html["realizar una actualización in situ del nodo de gestión"] a la versión 12.8 o posterior antes de utilizar NetApp Hybrid Cloud Control o la API REST para actualizar los servicios de administración, el software Element y el firmware de almacenamiento.
+
Si va a implementar un nuevo nodo de administración con la versión 12.8 o posterior, puede usar NetApp Hybrid Cloud Control o la API REST para realizar las actualizaciones de componentes en el orden que se indica en la documentación.<<sys_upgrade,secuencia de actualización del sistema>> .
...
* Has reservado tiempo suficiente para tus actualizaciones de link:task_hcc_upgrade_element_software.html["Software Element"] y link:task_hcc_upgrade_storage_firmware.html#storage-firmware-upgrade["firmware de almacenamiento"]. Cuando actualizas al software Element 12.5 o una versión posterior, el tiempo que tarda el proceso de actualización varía según la versión del software Element y las actualizaciones de firmware.
